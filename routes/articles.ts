...
const getAll = async (ctx: RouterContext, next: any) => {
  ...
  if (result.length) {
    const body: Post[] = result.map((post: any) => {
      ...
      // Change to https from http, and added ':3000'
      const links = {
        likes: `https://${ctx.host}:3000/api/v1/articles/${post.id}/likes`,
        fav: `https://${ctx.host}:3000/api/v1/articles/${post.id}/fav`,
        msg: `https://${ctx.host}:3000/api/v1/articles/${post.id}/msg`,
        self: `https://${ctx.host}:3000/api/v1/articles/${post.id}`,
      };
      return {
        id,
        title,
